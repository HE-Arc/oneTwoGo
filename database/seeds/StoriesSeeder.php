    ...
    public function run()
    {
      $id = DB::table('stories')->insertGetId(
        [
          ...
          'text' => 'Mourant, allongé sur le lit dans sa chambre, un vieillard distingue une merveilleuse odeur de tarte aux pommes qui ne lui est pas inconnu...
          Il demanda a son petit fils : "Petit, va donc me chercher une part de tarte aux pommes, elle sent tellement bon, ce serait un crime de m\'en priver... "
      ...
      $this->savePivot(1, $id);
      $this->savePivot(11, $id);
      $this->savePivot(65, $id);
      $this->savePivot(72, $id);
...
Lorsque sa mère tente de lui parler à travers ses créations, il les tue et les collecte dans une boîte. Après une perte tragique, il apprend enfin à connaître son histoire à travers un message caché qu\'il aurait dû lire il y a longtemps.',
        ...
        $this->savePivot(157, $id);
        $this->savePivot(158, $id);
...
Dorrit pense au monde extérieur et a hâte de le voir. Cet extrait vous présente la famille et sa vie en prison. Le roman explique comment ils réussissent à sortir et comment Dorrit n\'oublie jamais la gentillesse des personnes qui l\'ont aidée.',
        ...
        $this->savePivot(306, $id);
        $this->savePivot(308, $id);
        ...
        $id = DB::table('stories')->insertGetId(
          [
            ...
            'text' => 'Mme Mallard a des problèmes cardiaques qui pourraient m\'être un terme à son existence. Quand son mari meurt, les gens qui viennent lui donner cette nouvelle essaient de le faire gentiment. Quand elle est finalement informée, elle éclate en sanglots. Finalement, elle va dans sa chambre et s’enferme.
Cependant, tout en pensant à l’avenir, elle est enthousiasmée par l’idée de liberté qui pourrait découler de la mort de son mari. Au bout d\'une heure, la sonnette sonne à la porte et son mari est debout, bien vivant. Quand elle le voit, elle fait une crise cardiaque et meurt.',
        ...
        $this->savePivot(402, $id);
        $this->savePivot(405, $id);
